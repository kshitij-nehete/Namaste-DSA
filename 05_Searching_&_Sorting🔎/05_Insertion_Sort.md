...
_Note: Your original code had a syntax error in the while condition that I've fixed below:_
...
function insertionSort(arr) {
  ...
  for (let i = 1; i = 0 && arr[prev] > curr) {  // Fixed: removed extra bracket
      arr[prev + 1] = arr[prev]; // Shift element to the right
      prev--;
    }
